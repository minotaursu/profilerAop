...
public class Profiler {
    ...
    private static String toMetrics(Entry entry) {
        ...
        toMetrics(metricsMap, entry.subEntries);
        List<Metrics> metricss = new ArrayList<>();
        for (Map.Entry<String, Metrics> metricsEntry : metricsMap.entrySet()) {
            metricss.add(metricsEntry.getValue());
        }
        Collections.sort(metricss, new Comparator<Metrics>() {
            @Override
        ...
        for (Metrics metrics : metricss) {
            buffer.append('\n');
            buffer.append("+- ");
            buffer.append(metrics.getMessage());
            buffer.append(" ");
            buffer.append(metrics.getCount());
        }
        return buffer.toString();
    }

    private static void toMetrics(Map<String, Metrics> map, List<Entry> entries) {

        for (int i = 0; i < entries.size(); i++) {
            Entry subEntry = (Entry) entries.get(i);
            String subMessage = subEntry.getMessage();
            long duration = subEntry.getDuration();
            if (map.containsKey(subMessage)) {
                Metrics metrics = map.get(subMessage);
                metrics.addDuration(duration);
                metrics.incrCount();
                map.put(subMessage, metrics);
            } else {
                Metrics metrics = new Metrics();
                metrics.setMessage(subMessage);
                metrics.setDuration(duration);
                map.put(subMessage, metrics);
            }
            toMetrics(map, subEntry.subEntries);
        }
    }
    ...
    public static final class Entry {
        ...
        private Entry(Object message/*描述信息*/, Entry parentEntry/*父节点信息*/, Entry firstEntry/*第一个节点*/) {
            ...
            this.startTime = ProfilerSwitch.getInstance().isOpenProfilerNanoTime() ? System.nanoTime()
        ...
        private void release() {
            endTime = ProfilerSwitch.getInstance().isOpenProfilerNanoTime() ? System.nanoTime()
                : System.currentTimeMillis();
        }
